...
/**
 * Cria e gerencia o heartbeat para o sistema
 */
class HeartbeatSystem {
  ...
  emitirBatimento() {
    ...
    if (this.contadorBatimentos % 10 === 0) {
      this.logger.info(`💓 Heartbeat #${this.contadorBatimentos} - Sistema ativo há ${Math.floor((agora - this.inicioSistema) / 1000)}s`);
    } else {
      this.logger.info(`Heartbeat ${new Date().toISOString()} - Sistema ativo`);
    }
    
    // Verificar notificações pendentes (se existir o sistema)
