...
class FilaProcessador {
  ...
  iniciarMonitoramentoEventLoop() {
    ...
    setInterval(() => {
      ...
      this.registrador.info(`Event loop lag: ${lag}ms`);
  ...
  configurarProcessadores() {
    ...
    this.videoUploadQueue.process('upload-video', 3, async (job) => {
      const { tempFilename, chatId, messageId, mimeType, userPrompt, senderNumber, transacaoId } = job.data;
      ...
      try {
        ...
        if (erro.message.includes('SAFETY') || erro.message.includes('safety') || 
          ...
          // Notificar o usuário sobre outros tipos de erro (código existente)
          try {
    ...
    // 2. Processador para verificação do estado de processamento
      ...
      const { fileName, tempFilename, chatId, messageId, mimeType, userPrompt, senderNumber, transacaoId, uploadTimestamp } = job.data;
      
      try {
        this.registrador.info(`[Etapa 2] Verificando processamento do vídeo: ${fileName} (Job ${job.id})`);
        ...
        // Informar o usuário sobre o progresso após um tempo
        if (tentativas === 0 && Date.now() - uploadTimestamp > 20000) {
          try {
            await this.clienteWhatsApp.enviarMensagem(
              senderNumber, 
              "Seu vídeo está sendo processado... isso pode levar alguns minutos para vídeos longos ou complexos."
            );
          } catch (err) {
    ...
    this.videoAnalysisQueue.process('analyze-video', 3, async (job) => {
      const { 
        ...
        transacaoId, fileState, fileUri, fileMimeType 
      } = job.data;
      ...
      try {
        ...
        // Enviar resposta
        await this.clienteWhatsApp.enviarMensagem(senderNumber, `✅ *Análise do seu vídeo:*\n\n${resposta}`);
        this.registrador.info(`[Etapa 3] Resposta de vídeo enviada para ${senderNumber}`);
    ...
    this.videoQueue.process('process-video', 3, async (job) => {
      ...
      try {
        ...
        const uploadJob = await this.videoUploadQueue.add('upload-video', {
          tempFilename, chatId, messageId, mimeType, userPrompt, senderNumber, transacaoId
        });
